...
executeCommand(const QString &line)
{
  ...
  bool rc = eval(line, /*showError*/true, /*showResult*/log);
...
textChanged()
{
  ...
  colors_.resize(size_t(str.length()));

  for (const auto *token : tokens) {
    int  pos = token->pos();
    auto len = token->str().size();
...
{
  return CTclUtil::isCompleteLine(str.toStdString());
}
...
//bool        commandInside = false;
  ...
  for (int pos1 = commandPos + int(command.length()); pos1 < text.length(); ++pos1) {
    ...
    if      (token1->type() == CTclToken::Type::COMMAND) {
      ...
      pos1 = int(token1->pos()) + int(token1->str().length()); // move to end
    }
    else {
      if (lastOption != "")
...
getCommandWidget()
{
  return qobject_cast<CommandWidget *>(getCommand());
}

...
proc proietta { pt } {
  ...
  return [$::PVector create [expr {0.63*$::width + $sc*$x}] [expr {0.95*$::height - $sc*$y}] 0]
}
...
proc palazzo { pt } {
  ...
  if       {$tipo == 0} {
    ...
    for {set i [expr {$nr - 1}]} {$i >= 0} {incr i -1} {
      box3d [$::PVector create [$pt get x]             [expr {$i*4+4}] [$pt get z]] \
            [$::PVector create [expr {[$pt get x]+30}] [expr {$i*4+6}] [expr {[$pt get z]+30}]]
    }
    ...
    box3d [$::PVector create [expr {[$pt get x] +  1}] [expr {$h-10}] [expr {[$pt get z] +  1}]] \
          [$::PVector create [expr {[$pt get x] + 29}]             $h [expr {[$pt get z] + 29}]]
    box3d [$::PVector create [expr {[$pt get x] +  2}]             10 [expr {[$pt get z] + 22}]] \
          [$::PVector create [expr {[$pt get x] +  4}] [expr {$h-10}] [expr {[$pt get z] + 28}]]
    box3d [$::PVector create [expr {[$pt get x] + 22}]             10 [expr {[$pt get z] +  2}]] \
          [$::PVector create [expr {[$pt get x] + 28}] [expr {$h-10}] [expr {[$pt get z] +  4}]]
    box3d [$::PVector create [expr {[$pt get x] +  4}]             10 [expr {[$pt get z] +  4}]] \
          [$::PVector create [expr {[$pt get x] + 22}] [expr {$h-10}] [expr {[$pt get z] + 22}]]
    box3d [$::PVector create [expr {[$pt get x] +  2}]             10 [expr {[$pt get z] +  2}]] \
          [$::PVector create [expr {[$pt get x] +  8}] [expr {$h-10}] [expr {[$pt get z] +  8}]]
    ...
    if {$h > 50} {
      box3d [$::PVector create [expr {[$pt get x] +  2}] 50 [expr {[$pt get z] +  2}]] \
            [$::PVector create [expr {[$pt get x] + 28}] $h [expr {[$pt get z] + 28}]]
    }
    if {$h > 50} {
      box3d [$::PVector create [expr {[$pt get x] +  4}] 42 [expr {[$pt get z] +  4}]] \
            [$::PVector create [expr {[$pt get x] + 26}] 50 [expr {[$pt get z] + 26}]]
    }
    if {$h > 42} {
      box3d [$::PVector create [expr {[$pt get x] +  2}] 16 [expr {[$pt get z] +  2}]] \
            [$::PVector create [expr {[$pt get x] + 28}] 42 [expr {[$pt get z] + 28}]]
    }
    if {$h > 16} {
      box3d [$::PVector create [expr {[$pt get x] +  4}] 12 [expr {[$pt get z] +  4}]] \
    ...
    # giardino
    box3d $pt [$::PVector create [expr {[$pt get x] + 30}] \
                                 [expr {[$pt get y] +  1}] \
                                 [expr {[$pt get z] + 30}]]

    for {set i 0} {$i < 10} {incr i} {
      set pz [sb::map $i 0 10 0 22]
      set px [randIn 0 $pz]

      set pt0 [$::PVector create [expr {[$pt get x] + 26 - $px}] \
                                 [expr {[$pt get y] + 1}] \
                                 [expr {[$pt get z] + 26 - $pz + $px}]]

      albero $pt0
    }
  } elseif {$tipo == 7} {
    box3d [$::PVector create [expr {[$pt get x] +  5}] 32 [expr {[$pt get z] +  5}]] \
          [$::PVector create [expr {[$pt get x] + 25}] $h [expr {[$pt get z] + 25}]]
    box3d [$::PVector create [expr {[$pt get x] + 10}] 26 [expr {[$pt get z] + 10}]] \
          [$::PVector create [expr {[$pt get x] + 20}] 32 [expr {[$pt get z] + 20}]]
    box3d [$::PVector create [expr {[$pt get x] +  5}] 26 [expr {[$pt get z] + 24}]] \
          [$::PVector create [expr {[$pt get x] +  5}] 32 [expr {[$pt get z] + 25}]]
    box3d [$::PVector create [expr {[$pt get x] +  5}] 26 [expr {[$pt get z] + 18}]] \
          [$::PVector create [expr {[$pt get x] +  5}] 32 [expr {[$pt get z] + 19}]]
    box3d [$::PVector create [expr {[$pt get x] +  5}] 26 [expr {[$pt get z] + 11}]] \
          [$::PVector create [expr {[$pt get x] +  5}] 32 [expr {[$pt get z] + 12}]]
    box3d [$::PVector create [expr {[$pt get x] + 24}] 26 [expr {[$pt get z] +  5}]] \
          [$::PVector create [expr {[$pt get x] + 25}] 32 [expr {[$pt get z] +  6}]]
    box3d [$::PVector create [expr {[$pt get x] + 18}] 26 [expr {[$pt get z] +  5}]] \
          [$::PVector create [expr {[$pt get x] + 19}] 32 [expr {[$pt get z] +  6}]]
    box3d [$::PVector create [expr {[$pt get x] + 11}] 26 [expr {[$pt get z] +  5}]] \
          [$::PVector create [expr {[$pt get x] + 12}] 32 [expr {[$pt get z] +  6}]]
    box3d [$::PVector create [expr {[$pt get x] +  5}] 26 [expr {[$pt get z] +  5}]] \
          [$::PVector create [expr {[$pt get x] +  6}] 32 [expr {[$pt get z] +  6}]]

    cilindro3d [$::PVector create [$pt get x] 6 [$pt get z]] 20 25 15 0

    box3d [$::PVector create [expr {[$pt get x] + 10}] 0 [expr {[$pt get z] + 10}]] \
          [$::PVector create [expr {[$pt get x] + 20}] 6 [expr {[$pt get z] + 20}]]
    box3d [$::PVector create [expr {[$pt get x] +  5}] 0 [expr {[$pt get z] + 24}]] \
          [$::PVector create [expr {[$pt get x] +  5}] 6 [expr {[$pt get z] + 25}]]
    box3d [$::PVector create [expr {[$pt get x] +  5}] 0 [expr {[$pt get z] + 18}]] \
          [$::PVector create [expr {[$pt get x] +  5}] 6 [expr {[$pt get z] + 19}]]
    box3d [$::PVector create [expr {[$pt get x] +  5}] 0 [expr {[$pt get z] + 11}]] \
          [$::PVector create [expr {[$pt get x] +  5}] 6 [expr {[$pt get z] + 12}]]
    box3d [$::PVector create [expr {[$pt get x] + 24}] 0 [expr {[$pt get z] +  5}]] \
          [$::PVector create [expr {[$pt get x] + 25}] 6 [expr {[$pt get z] +  6}]]
    box3d [$::PVector create [expr {[$pt get x] + 18}] 0 [expr {[$pt get z] +  5}]] \
          [$::PVector create [expr {[$pt get x] + 19}] 6 [expr {[$pt get z] +  6}]]
    box3d [$::PVector create [expr {[$pt get x] + 11}] 0 [expr {[$pt get z] +  5}]] \
          [$::PVector create [expr {[$pt get x] + 12}] 6 [expr {[$pt get z] +  6}]]
    box3d [$::PVector create [expr {[$pt get x] +  5}] 0 [expr {[$pt get z] +  5}]] \
          [$::PVector create [expr {[$pt get x] +  6}] 6 [expr {[$pt get z] +  6}]]

    set insegne 1
  } elseif {$tipo == 8} {
    box3d [$::PVector create [expr {[$pt get x] +  2}]              6   [expr {[$pt get z] +  2}]] \
          [$::PVector create [expr {[$pt get x] + 18}] [expr {$h - 10}] [expr {[$pt get z] + 18}]]
    box3d [$::PVector create [expr {[$pt get x]     }]              0   [expr {[$pt get z] + 23}]] \
          [$::PVector create [expr {[$pt get x] +  2}] [expr {$h     }] [expr {[$pt get z] + 28}]]
    box3d [$::PVector create [expr {[$pt get x]     }]              0   [expr {[$pt get z] + 18}]] \
          [$::PVector create [expr {[$pt get x] +  2}] [expr {$h -  5}] [expr {[$pt get z] + 23}]]
    box3d [$::PVector create [expr {[$pt get x]     }]              0   [expr {[$pt get z] +  7}]] \
          [$::PVector create [expr {[$pt get x] +  2}] [expr {$h -  5}] [expr {[$pt get z] + 12}]]
    box3d [$::PVector create [expr {[$pt get x]     }]              0   [expr {[$pt get z] +  2}]] \
          [$::PVector create [expr {[$pt get x] +  2}] [expr {$h     }] [expr {[$pt get z] +  7}]]
    box3d [$::PVector create [expr {[$pt get x] + 23}]              0   [expr {[$pt get z]     }]] \
          [$::PVector create [expr {[$pt get x] + 28}] [expr {$h     }] [expr {[$pt get z] +  2}]]
    box3d [$::PVector create [expr {[$pt get x] + 18}]              0   [expr {[$pt get z]     }]] \
          [$::PVector create [expr {[$pt get x] + 23}] [expr {$h -  5}] [expr {[$pt get z] +  2}]]
    box3d [$::PVector create [expr {[$pt get x] +  7}]              0   [expr {[$pt get z]     }]] \
          [$::PVector create [expr {[$pt get x] + 12}] [expr {$h -  5}] [expr {[$pt get z] +  2}]]
    box3d [$::PVector create [expr {[$pt get x] +  2}]              0   [expr {[$pt get z]     }]] \
          [$::PVector create [expr {[$pt get x] +  7}] [expr {$h     }] [expr {[$pt get z] +  2}]]
...
proc albero { pt0 } {
  ...
  $pt1 set x [expr {[$pt1 get y] + 4}]

  set pt2 $pt1 ; # dup

  $pt2 set x [expr {[$pt2 get x] + 2}]
  $pt2 set x [expr {[$pt2 get z] - 2}]
...
proc semaforoz { pt0 } {
  vlinea [proietta [$::PVector create [$pt0 get x] 0 [expr {[$pt0 get z] -2 }]]] \
         [proietta [$::PVector create [$pt0 get x] 5 [expr {[$pt0 get z] -2 }]]]

  vlinea [proietta [$::PVector create [$pt0 get x] 0 [expr {[$pt0 get z] -18}]]] \
         [proietta [$::PVector create [$pt0 get x] 5 [expr {[$pt0 get z] -18}]]]

  vlinea [proietta [$::PVector create [$pt0 get x] 4 [expr {[$pt0 get z]  -2}]]] \
         [proietta [$::PVector create [$pt0 get x] 4 [expr {[$pt0 get z] -18}]]]

  vlinea [proietta [$::PVector create [$pt0 get x] 5 [expr {[$pt0 get z]  -2}]]] \
         [proietta [$::PVector create [$pt0 get x] 5 [expr {[$pt0 get z] -18}]]]
...
proc striscia { del p1 p2 p3 p4 } {
  ...
  while {$pp < $del} {
    linea [$p1 get x] [expr {[$p2 get y] + $pp*$r1}] [$p3 get x] [expr {[$p4 get y] + $pp*$r2}]
...
proc box3d { pt1 pt2 } {
  ...
  striscia [expr {[$pt2 get y] - [$pt1 get y]}] $p3 $p4 $p5 $p6
}
...
proc cilindro3d { pt h nrseg rad contorno } {
  ...
  for {set i 0} {$i < $nrseg} {incr i} {
    ...
    if {[[$pv get value $i 0] get x] <= [[$pv get value $j 0] get x]} {
      if {$i > 0} {
        $::lista exec agg 3 [[$pv get value $j 0] get x] [[$pv get value $j 0] get y]
        $::lista exec agg 3 [[$pv get value $i 0] get x] [[$pv get value $i 0] get y]
        $::lista exec agg 3 [[$pv get value $i 1] get x] [[$pv get value $i 1] get y]
        $::lista exec agg 4 [[$pv get value $j 1] get x] [[$pv get value $j 1] get y]
      }
    }
  }
  for {set i 0} {$i < $nrseg} {incr i} {
  ...
  $::lista exec agg 5 [[$pv get value 0 0] get x] [[$pv get value 0 0] get y]
  for {set i 0} {$i < $nrseg} {incr i} {
    set j [expr {($i + $nrseg - 1) % $nrseg}]
    ...
    if {[[$pv get value $i 0] get x] <= [[$pv get value $j 0] get x]} {
      ...
      if {$i > 0} {
        vdlinea [$pv get value $j 0] [$pv get value $i 0]
        vdlinea [$pv get value $j 1] [$pv get value $i 1]

        if {sin($alfa) > 0} {
          striscia $h [$pv get value $i 0] [$pv get value $i 1] \
...
proc pilotis { pt0 dex dez h } {
  ...
  for {set i $nrx} {$i >= 0} {incr i -1} {
    box3d [$::PVector create [expr {[$pt0 get x] + $i*$delx    }]  0 [$pt0 get z]             ] \
          [$::PVector create [expr {[$pt0 get x] + $i*$delx + 1}] $h [expr {[$pt0 get z] + 1}]]
  }
}
...
proc rettangolo_bianco { pt1 pt2 pt3 pt4 } {
  ...
  $::lista exec agg 3 [$p2 get x] [$p2 get y]
  $::lista exec agg 3 [$p3 get x] [$p3 get y]
  $::lista exec agg 4 [$p4 get x] [$p4 get y]
...
proc rettf { sx sy ex ey } {
  ...
  for {set i 0} {$i < min($dex, $dey)} {incr i 6} {
    $::lista exec agg 1 $sx               [expr {$sy + $i}]
    $::lista exec agg 1 [expr {$sx + $i}] $sy
  }

  if {$dex > $dey} {
    for {set i 0} {$i < $dex - $dey} {incr i 6} {
      $::lista exec agg 1 [expr {$sx + $i       }] $ey
      $::lista exec agg 1 [expr {$sx + $dey + $i}] $sy
    }
  }
  else {
    for {set i 0} {$i < $dey - $dex} {incr i 6} {
      $::lista exec agg 1 $sx [expr {$sy + $dex + $i}]
      $::lista exec agg 1 $ex [expr {$sy + $i)}]
    }
  }
...
set ::PVector [sb::class PVector]

$::PVector proc init { v x y z } {
  $v set x $x
  $v set y $y
  $v set z $z
}
...
$::CListA proc _draw { l el } {
  ...
  if       {$m == 0} {
    ...
    $::renderer exec draw.line [list $::penx $::peny] \
        [list [$mat get value $el 1] [$mat get value $el 2]]
    ...
    $::renderer set draw.rect $::penx $::peny \
      ...
      [expr {[$mat get value $el 2] - $::peny}]
  } elseif {$m == 3} {
    # vertice poligono pieno
    set pv1 [$pv get value $nrv]
    $pv1 set x [$mat get value $el 1]
    $pv1 set y [$mat get value $el 2]
    $pv set value $nrv $pv1

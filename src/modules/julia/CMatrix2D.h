 ...
 ~CMatrix2D() { }
  ...
  bool isIdentity() const {
    return isInnerIdentity() && isTranslateIdentity() &&
           isZero(m20_) && isZero(m21_) && realEq(m22_, 1.0);
  }

  bool isInnerIdentity() const {
    return realEq(m00_, 1.0) && isZero(m01_     ) &&
           isZero(m10_     ) && realEq(m11_, 1.0);
  }

  bool isTranslateIdentity() const {
    return isZero(m02_) && isZero(m12_);
  }

  bool isInnerRotation() const {
    return ! isZero(m01_) && ! isZero(m10_);
  }
